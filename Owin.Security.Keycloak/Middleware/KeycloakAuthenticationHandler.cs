...
using Constants = Keycloak.IdentityModel.Constants;
...
namespace Owin.Security.Keycloak.Middleware
{
    internal class KeycloakAuthenticationHandler : AuthenticationHandler<KeycloakAuthenticationOptions>
    {
        ...
        public override async Task<bool> InvokeAsync()
        {
            ...
            var callbackUri = await KeycloakIdentity.GenerateLoginCallbackUriAsync(Options, Request.Uri.ToReverseProxyRedirectUri());

            if (!Options.ForceBearerTokenAuth && Request.Uri.GetLeftPart(UriPartial.Path).ToReverseProxyRedirectUrl() == callbackUri.ToString().ToReverseProxyRedirectUrl())
            {
        ...
        private void HandleAuthError(AuthorizationResponse authResult)
        {
            ...
            if (!string.IsNullOrWhiteSpace(Options.AuthResponseErrorRedirectUrl))
            {
                ...
                if (Uri.IsWellFormedUriString(authResponseErrorRedirectUrl, UriKind.Relative))
                {
                    ...
                    string baseAddress = string.Format("{0}{1}", Request.Uri.GetLeftPart(UriPartial.Authority).ToReverseProxyRedirectUrl(), Options.VirtualDirectory);
        ...
        private async Task LoginRedirectAsync(AuthenticationProperties properties)
        {
            ...
            properties.RedirectUri = properties.RedirectUri.ToReverseProxyRedirectUrl();
            ...
            var url = (await KeycloakIdentity.GenerateLoginUriAsync(Options, Request.Uri.ToReverseProxyRedirectUri(), state)).ToString();
        ...
        private async Task LogoutRedirectAsync()
        {
            ...
            Response.Redirect(
                (await
                    KeycloakIdentity.GenerateLogoutUriAsync(Options, Request.Uri.ToReverseProxyRedirectUri()))
                    .ToString());
        }
